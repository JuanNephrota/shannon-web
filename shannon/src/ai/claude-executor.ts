...
export async function runClaudePrompt(
  ...
  // Ensure node is in PATH for the SDK to spawn Claude Code CLI
  // This fixes "spawn node ENOENT" errors when running from web UI
  const nodeDir = path.dirname(process.execPath);
  const currentPath = process.env.PATH || '';
  const enhancedPath = currentPath.includes(nodeDir)
    ? currentPath
    : `${nodeDir}:${currentPath}`;
